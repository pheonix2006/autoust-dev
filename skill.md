...
Create or reuse `data/homework/<COURSE>/<assignment>/`. First investigate the
...
For a new installation, clone the full repository, not just this file. Use the
user's chosen directory; if the current project is empty, clone there with
`git clone <repo> .`. If the destination is ambiguous or nonempty, resolve the
location first. Do not silently default to a Desktop or home-directory sample.
